    ...
    if user == "3":
        ...
        while True:
            ...
            if check_valid_num(choose, "2"):
                # go on 
                converted = bin_to_den(choose)

                print(f"Binary {choose} = Denary {converted}")
                break
            else:
                print(f"{choose} is not a valid binary number. ")
            # validate using is_valid_num(choose, "2")

    elif user == "2":
         while True:
            choose = input("Enter what denary number you want to convert ")
            if check_valid_num(choose, "10"):
                converted = den_to_bin(choose)
                print(f"denary {choose} = binary {converted}")
                break
            else:
                print(f"{choose} is not a valid denary number. ")